...
class Paintr {
    constructor() {
        ...
        this.ctx = this.canvas.getContext('2d', { willReadFrequently: true }); // Needed for frequent getImageData
        this.currentTool = 'pencil';
        this.isDrawing = false;
        this.currentColor = '#CCCCCC'; // Light gray default color
        ...
        this.startX = 0;
        this.startY = 0;
        this.snapshot = null; // For shape drawing previews
        this.history = [];
        this.historyIndex = -1;
        this.maxHistorySize = MAX_HISTORY_SIZE;
        this.polygonPoints = []; // For polygon tool
        this.airbrushInterval = null; // For airbrush tool
        this.selection = null;
        this.curvePoints = [];
        ...
        this.fileInput = this.createFileInput(); // Create file input element

        this.initializeCanvas();
        this.setupEventListeners();
        this.setupThemeToggle();
        this.setupColorModeToggle();
        this.setupMenuActions(); // Replaces setupFileHandlers
        this.setupKeyboardShortcuts();
        this.updateColorDisplay(this.currentColor);
        this.setActiveTool('pencil'); // Set default active tool visually
    ...
    initializeCanvas() {
        ...
        // Status bar height might vary slightly, get it dynamically
        ...
        // Preserve drawing dataURL before resize
        ...
        if (currentDrawing) {
            // Use a flag to prevent immediate re-saving which would break undo
            this.loadImageDataURL(currentDrawing, false); 
        } else if (this.history.length === 0) { // Only save initial state if history is empty
             this.saveState(); 
        }
    ...
    setupEventListeners() {
        ...
        this.canvas.addEventListener('click', this.handleClick.bind(this)); // For single-click tools
    ...
    setupThemeToggle() {
        this.themeToggle.addEventListener('click', () => {
            ...
            // Update background color if needed (e.g., for eraser)
            this.bgColor = isLight ? '#FFFFFF' : '#000000'; 
             // Redraw canvas background if it's currently showing (e.g., after clear)
             // This requires checking if the current state IS the background color
             // Might be simpler to just stick with black/white bg regardless of theme for simplicity
             // Let's stick with the initial black bg for now.
            this.bgColor = '#000000'; // Keep canvas bg black for consistency
            this.updateColorDisplay(this.currentColor); // Update swatch borders
        });
    }
    ...
    setActiveTool(tool) {
        if (!tool) return;
        // Stop ongoing actions like airbrush or polygon drawing
        this.stopAirbrush();
        if (this.currentTool === 'polygon' && this.polygonPoints.length > 0) {
            this.drawPolygon(true); // Finalize polygon if switching tool
        }
        this.polygonPoints = [];
        ...
        if (toolButton) {
            ...
            if (tool === 'pencil') {
                ...
                this.brushSize = 25; // Set default airbrush size to 25px
            }
    ...
    handlePolygonClick(x, y) {
        ...
        if (this.polygonPoints.length > 1) {
            ...
            // Draw the first two sides
            for (let i = 1; i < this.polygonPoints.length; i++) {
                this.ctx.lineTo(this.polygonPoints[i].x, this.polygonPoints[i].y);
            }
            
            // If we have 2 points, show a preview of where the third point would complete the triangle
            if (this.polygonPoints.length === 2) {
                ...
                // Draw the preview line with a dashed style
        ...
        // Complete the triangle after 3 points
    ...
    drawPolygon(finalize = false) {
        ...
        // Fill the triangle if we're finalizing
        if (finalize) {
            this.ctx.fill();
        }
    ...
    startAirbrush() {
        ...
        this.airbrushInterval = setInterval(() => {
            ...
            // Create a more evenly distributed circular spray pattern
            for (let i = 0; i < 25; i++) {
                // Use golden angle for even distribution
                const goldenAngle = Math.PI * (3 - Math.sqrt(5));
                const radius = Math.sqrt(i / 25) * size * 2; // Double the radius
        ...
        }, 25); // Reduced from 50ms to 25ms for faster spray rate
    }
    ...
    addText(x, y) {
        ...
        if (text) {
            this.ctx.font = `${this.brushSize * 5}px 'MS Sans Serif', sans-serif`; // Size relative to brushSize? 
            this.ctx.fillStyle = this.currentColor;
    ...
    clearCanvas() {
        this.ctx.fillStyle = this.bgColor;
        this.ctx.fillRect(0, 0, this.canvas.width, this.canvas.height);
        // Don't save state here, let caller decide (e.g. import)
    }
        ...
        if (file) {
            ...
            reader.onload = (event) => {
                ...
                img.onload = () => {
                    ...
                    this.saveState(); // Save the imported image as the new base state
                };
    ...
    setupKeyboardShortcuts() {
        document.addEventListener('keydown', (e) => {
            ...
            // Handle number keys for brush size
    ...
    pickColor(x, y) {
        try {
            ...
            // Important: Alpha is pixelData[3]. Ignore transparent picks?
            if (pixelData[3] === 0) return; // Ignore transparent clicks

            const hexColor = `#${("000000" + ((pixelData[0] << 16) | (pixelData[1] << 8) | pixelData[2]).toString(16)).slice(-6)}`;
            this.setCurrentColor(hexColor);
            
            // Optional: Switch back to previous tool after picking
            // this.setActiveTool('pencil'); 
        } catch (error) {
             console.error("Error picking color:", error); 
             // Likely due to tainted canvas from cross-origin image
        }
    }

    // Simple Flood Fill Implementation (recursive, can cause stack overflow on large areas)
    // A queue-based (BFS) approach is generally better for performance and safety.
    floodFill(startX, startY, fillColor) {
        ...
        if (!targetColor) return; // Invalid fill color
        ...
        while (queue.length > 0) {
            ...
            // Check if the current pixel has the start color
    ...
    rgbToHex(rgbString) {
        if (!rgbString || !rgbString.startsWith('rgb')) return rgbString; // Assume already hex or other format
    ...
    // Add new method for tool preview
    showToolPreview(e) {
        const { x, y } = this.getMousePos(e);
        ...
        // Set cursor style on canvas instead of creating a preview element
        this.canvas.style.cursor = this.getCursorForTool(this.currentTool);
        
        // Only show preview for tools that need it
    ...
    finalizeSelection() {
        ...
        // Create a new canvas for the selection
        ...
        // Store the selection for potential move/copy operations
        this.currentSelection = selectionCanvas;
        this.selection = null;
    }

    // Curve Tool Implementation
    startCurve(x, y) {
    ...
    updateRoundedRect(x, y) {
        ...
        const radius = Math.min(Math.abs(width), Math.abs(height)) * 0.2; // 20% of smaller dimension
